...
class TripLocationThumbnailUploader {
  ...
  private async deleteExistingThumbnail(locationId: number): Promise<void> {
    try {
      // 데이터베이스에서 locationId에 해당하는 썸네일 조회
      const [existingLocation] = await this.db.query<RowDataPacket[]>(
        "SELECT thumbnail FROM trip_location_tb WHERE location_id = ?",
        [locationId]
      );

      if (existingLocation.length > 0 && existingLocation[0].thumbnail) {
        // 기존 썸네일 URL에서 파일명을 추출
        const thumbnailUrl = existingLocation[0].thumbnail;
  ...
  // 위치 썸네일 업로드 미들웨어 생성
  createThumbnailUploadMiddleware(locationId: number) {
    return multer({
      storage: multerS3({
        ...
        key: async (
          ...
          // 기존 썸네일 삭제
          await this.deleteExistingThumbnail(locationId);

          // 새로운 파일 이름 생성 (시간을 기준으로 고유한 이름)
          const timestamp = Date.now();
          const filename = `image/trip_location/${locationId}_${timestamp}.png`;
          cb(null, filename);
        },
  ...
  async updateThumbnailUrl(
    ...
    try {
      ...
      await connection.query(
        "UPDATE trip_location_tb SET thumbnail = ? WHERE location_id = ?",
        [thumbnailUrl, locationId]
      );
...
export default TripLocationThumbnailUploader; // 클래스 내보내기
